...
-- ─────────────────────────────────────────────────────────────────────
-- Tables + columns that used to be re-defined here as a "mirror" of the
-- Drizzle schema (the local DB wasn't baselined) are now owned by the
-- versioned migrations (0004 + 0007). post-schema no longer re-creates them.
-- What stays below is ONLY what migrations can't model: retirement DROPs,
-- the triage_status NOT NULL alignment, the partial public-wall index, plus
-- (further down) triggers, RPC functions, and CHECK constraints.
--
-- DEPLOY NOTE: on an already-provisioned DB (production), run `db:adopt`
-- ONCE so migration 0007 is recorded as applied (those objects already
-- exist there); then deploy as usual — db:migrate, then re-apply this file.
-- ─────────────────────────────────────────────────────────────────────
...
-- partnership_offers.lead_id index — the table is created by migration 0007 but
-- this lookup index isn't modeled in the Drizzle schema, so post-schema owns it.
