    ...
    return (
      <div className="flex min-h-[70vh] items-center justify-center px-4 py-16">
        <div className="w-full max-w-sm rounded-2xl border border-border/60 bg-card p-8 text-center shadow-sm">
          ...
          <h1 className="text-subhead font-bold">عرض شراكة خاص</h1>
      ...
      <div className="container mx-auto px-4">
        <article className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-border/60 bg-card shadow-sm">
          ...
          <header className="border-b border-border/50 bg-gradient-to-br from-primary/[0.08] to-accent/[0.05] px-8 py-10 text-center sm:px-12">
            <div className="mx-auto mb-4 inline-flex items-center gap-1.5 rounded-full border border-primary/25 bg-primary/5 px-3 py-1 text-micro font-semibold text-primary">
          ...
          <div className="space-y-8 px-8 py-10 sm:px-12">
            {/* Intro */}
            {offer.intro && (
              <p className="text-body text-foreground/90">{offer.intro}</p>
            )}

            {/* Body */}
            {offer.body && (
              <div className="whitespace-pre-wrap text-body text-foreground/85">
                {offer.body}
              </div>
            )}
            ...
            {offer.packages.length > 0 && (
              <div className="space-y-4">
                <h2 className="text-lead font-bold">الباقات المقترحة</h2>
                <div className="grid gap-4 sm:grid-cols-2">
                  {offer.packages.map((pkg, i) => (
                    <div key={i} className="rounded-2xl border border-border/60 bg-background/50 p-5">
                      ...
                      {pkg.price_range && (
                        <p className="mt-2 text-caption font-semibold text-primary">{pkg.price_range}</p>
                      )}
                      {pkg.deliverables.length > 0 && (
                        <ul className="mt-3 space-y-1.5 border-t border-border/40 pt-3">
                          {pkg.deliverables.map((d, j) => (
                            <li key={j} className="flex items-start gap-2 text-caption text-foreground/85">
            ...
            <div className="rounded-2xl bg-primary/[0.04] p-6 text-center ring-1 ring-primary/15">
              ...
              {offer.contact_email && (
                <a
                  ...
                  className="mt-4 inline-flex items-center justify-center gap-2 rounded-full bg-primary px-6 py-2.5 text-caption font-medium text-primary-foreground transition-opacity hover:opacity-90"
                >
          ...
          <footer className="border-t border-border/50 px-8 py-5 text-center text-micro text-muted-foreground">
