    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-[13px] font-semibold text-foreground">{lead.name}</p>
          <p className="truncate text-[11px] text-muted-foreground">{lead.country}</p>
        </div>
        {lead.fit_score != null ? (
      ...
      <div className="mt-2 flex items-center justify-between">
        ...
        <span className="text-[10px] text-muted-foreground">{ageLabel(lead.created_at)}</span>
      </div>
    </Link>
  )
}
...
function Stat({
  ...
  const toneCls =
    tone === "good" ? "bg-emerald-50 text-emerald-700" : tone === "warn" ? "bg-amber-50 text-amber-700" : "bg-muted text-muted-foreground"
  return (
