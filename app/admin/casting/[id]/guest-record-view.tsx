      ...
      <div className="rounded-2xl border border-border/80 bg-card p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold tracking-tight text-foreground">{app.name}</h1>
            <div className="mt-1 flex flex-wrap items-center gap-2 text-[12px] text-muted-foreground">
              <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {app.country}</span>
              <a href={`mailto:${app.email}`} className="inline-flex items-center gap-1 hover:text-foreground">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-1.5">
          ...
          <button
            ...
            disabled={busy !== null}
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[12px] font-medium transition-all disabled:opacity-60 ${
              app.status === "consider_later" ? "border-amber-300 bg-amber-500 text-white" : "border-border bg-card text-muted-foreground hover:border-amber-200 hover:text-amber-700"
            }`}
          >
          ...
          <button
            ...
            disabled={busy !== null}
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[12px] font-medium transition-all disabled:opacity-60 ${
              app.status === "rejected" ? "border-rose-300 bg-rose-600 text-white" : "border-border bg-card text-muted-foreground hover:border-rose-200 hover:text-rose-700"
            }`}
          >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[14px] font-bold text-foreground">{record.eir.working_title}</p>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <span className="rounded-md bg-card px-2 py-0.5 text-[11px] font-medium text-indigo-700">{PHASE_LABEL[record.eir.phase] || record.eir.phase}</span>
...
function Stat({ icon: Icon, label, value, sub, valueClass }: { icon: React.ElementType; label: string; value: string; sub: string; valueClass?: string }) {
  return (
    <div className="rounded-2xl border border-border/80 bg-card p-3.5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      ...
      <div className={`mt-1 truncate text-[20px] font-bold leading-tight ${valueClass || "text-foreground"}`}>{value}</div>
  ...
  const field = (label: string, v: string | null) =>
    v ? (
      <div>
        ...
        <p className="mt-0.5 whitespace-pre-wrap text-[13px] leading-relaxed text-muted-foreground">{v}</p>
      </div>
    ) : null
  return (
    <div className="space-y-4">
      ...
      <SectionCard title="ملف الترشيح" icon={Brain}>
        {a?.status === "generating" ? (
          ...
          <div className="text-center">
            <p className="mb-3 text-[13px] text-muted-foreground">شغّل التقييم للحصول على بحث حيّ عن المتقدم وقراءة تحريرية كاملة.</p>
  ...
  return (
    <div className="space-y-4">
      <SectionCard title="درجات الترشيح" icon={Target}>
        ...
        {a.fit_summary && <p className="mt-3 border-t border-border pt-3 text-[13px] leading-relaxed text-muted-foreground">{a.fit_summary}</p>}
      </SectionCard>
  ...
  return (
    ...
    >
      {c?.status === "ready" && c.proposed_episode_title ? (
        <div className="space-y-3">
          <div>
            <p className="text-[17px] font-bold text-foreground">{c.proposed_episode_title}</p>
            {c.title_alternatives.length > 0 && (
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {c.title_alternatives.map((t, i) => (
                  <span key={i} className="rounded-md bg-muted px-2 py-0.5 text-[11px] text-muted-foreground">{t}</span>
                ))}
              </div>
            )}
          </div>
          {c.episode_hook && <p className="border-s-2 border-orange-300 ps-3 text-[13px] italic leading-relaxed text-muted-foreground">“{c.episode_hook}”</p>}
          ...
          {c.suggested_opening_question && (
            <div className="rounded-xl bg-indigo-50/60 p-3">
              ...
              <p className="mt-0.5 text-[13px] font-medium leading-relaxed text-foreground">{c.suggested_opening_question}</p>
            </div>
          )}
          {c.suggested_core_questions.length > 0 && (
            <div>
              <p className="mb-1.5 text-[11px] font-medium text-muted-foreground">الأسئلة الجوهرية</p>
  ...
  return (
    ...
    >
      <div className="mb-2 flex flex-wrap gap-1.5">
        {MSG_TYPES.map((t) => (
          <button
            ...
            className={`rounded-lg px-2.5 py-1 text-[11.5px] font-medium ${type === t.key ? "bg-foreground text-white" : "bg-muted text-muted-foreground hover:bg-muted-foreground/10"}`}
          >
            {t.label}
          </button>
        ))}
        ...
        {(["formal", "warm"] as const).map((tn) => (
          <button
            ...
            className={`rounded-lg px-2.5 py-1 text-[11.5px] font-medium ${tone === tn ? "bg-indigo-600 text-white" : "bg-muted text-muted-foreground hover:bg-muted-foreground/10"}`}
          >
            {tn === "formal" ? "رسمي" : "ودّي"}
          </button>
        ))}
      ...
      <textarea
        ...
        className="w-full resize-y rounded-xl border border-border p-3 text-[12.5px] leading-relaxed"
      />
      <div className="mt-2 flex items-center justify-between">
        <span className="text-[11px] text-muted-foreground">يُرسَل إلى {app.email}</span>
        <button
          onClick={send}
          disabled={busy !== null || !shown.trim()}
          className="inline-flex items-center gap-1.5 rounded-lg bg-foreground px-3.5 py-1.5 text-[12.5px] font-semibold text-white hover:bg-foreground/90 disabled:opacity-50"
        >
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {!f ? (
          <div className="text-center">
            <p className="mb-3 text-[13px] text-muted-foreground">أنشئ رابط استبيان تحضير وشاركه مع الضيف.</p>
          ...
          <div className="space-y-2.5">
            {link ? (
              <div className="flex items-center gap-2 rounded-lg bg-muted p-2">
                <code className="min-w-0 flex-1 truncate text-[11.5px] text-muted-foreground" dir="ltr">{link}</code>
                <button
                  onClick={() => { navigator.clipboard.writeText(link); setCopied(true); setTimeout(() => setCopied(false), 1500) }}
                  className="shrink-0 rounded-md bg-card px-2 py-1 text-[11px] text-muted-foreground hover:bg-muted"
                >
                  {copied ? <CheckCheck className="h-3.5 w-3.5 text-emerald-700" /> : <Copy className="h-3.5 w-3.5" />}
                </button>
              </div>
            ) : (
              <p className="text-[12px] text-muted-foreground">الرابط سرّي ولا يُعرض بعد الإنشاء — أعد توليده للحصول على رابط جديد للمشاركة.</p>
            )}
            <div className="flex flex-wrap gap-1.5">
              <button onClick={regenerate} disabled={busy !== null} className="rounded-lg border border-border px-2.5 py-1 text-[11.5px] text-muted-foreground hover:bg-muted disabled:opacity-60">
                {busy === "prep:regen" ? "…" : "إعادة توليد الرابط"}
              </button>
              {f.status !== "revoked" && f.status !== "locked" && (
                <button onClick={() => action("lock", "lock")} disabled={busy !== null} className="rounded-lg border border-border px-2.5 py-1 text-[11.5px] text-muted-foreground hover:bg-muted disabled:opacity-60">قفل</button>
              )}
              {f.status === "locked" && (
                <button onClick={() => action("unlock", "unlock")} disabled={busy !== null} className="rounded-lg border border-border px-2.5 py-1 text-[11.5px] text-muted-foreground hover:bg-muted disabled:opacity-60">فتح</button>
              )}
              {f.status !== "revoked" && (
...
function TasksCard({
  ...
  return (
    <SectionCard
      ...
      icon={CheckCircle2}
      action={<button onClick={() => setAdding((s) => !s)} className="text-muted-foreground hover:text-indigo-700"><Plus className="h-4 w-4" /></button>}
    >
      {adding && (
        <div className="mb-3 flex gap-1.5">
          <input value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addTask()} autoFocus placeholder="مهمة جديدة…" className="flex-1 rounded-lg border border-border px-2.5 py-1 text-[12.5px]" />
      ...
      {open.length === 0 && done.length === 0 ? (
        ...
        <div className="space-y-1.5">
          {open.map((t) => {
            ...
            return (
              <div key={t.id} className="flex items-start gap-2 rounded-xl border border-border bg-card p-2.5">
                <button onClick={() => complete(t)} disabled={busy !== null} className="mt-0.5 text-muted-foreground/50 transition-colors hover:text-emerald-700" title="إنجاز">
          ...
          {done.length > 0 && <p className="pt-1 text-[11px] text-muted-foreground">{done.length} مهمة منجزة</p>}
        </div>
      )}
    </SectionCard>
  )
  ...
  return (
    <SectionCard title="ملاحظات الفريق" icon={MessageSquare}>
      <div className="mb-2.5 flex gap-1.5">
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={2} placeholder="اكتب ملاحظة…" className="flex-1 resize-none rounded-lg border border-border px-2.5 py-1.5 text-[12.5px]" />
