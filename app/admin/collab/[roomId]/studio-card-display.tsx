...
const BUCKET_STYLES: Record<string, string> = {
  opening: "bg-sky-500/10 text-sky-700 border-sky-500/20",
  deep: "bg-violet-500/10 text-violet-700 border-violet-500/20",
  escalation: "bg-orange-500/10 text-orange-700 border-orange-500/20",
  surprise: "bg-pink-500/10 text-pink-700 border-pink-500/20",
  backup: "bg-slate-500/10 text-slate-700 border-slate-500/20",
  recovery: "bg-emerald-500/10 text-emerald-700 border-emerald-500/20",
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">#{card.sort_order + 1}</span>

        <div className="flex items-center gap-2 mr-auto">
          {card.clip_potential && (
            <span title="محتوى مقطع" className="flex items-center gap-1 text-amber-700/70">
              <Sparkles className="h-3.5 w-3.5" />
            </span>
          )}
          {card.quote_potential && (
            <span title="اقتباس" className="flex items-center gap-1 text-violet-700/70">
              <Quote className="h-3.5 w-3.5" />
            </span>
          )}
          {card.emotional_peak && (
            <span title="ذروة عاطفية" className="flex items-center gap-1 text-red-700/70">
      ...
      {/* Title */}
      <h2 className="text-sm font-medium text-muted-foreground">{card.short_title}</h2>
      ...
      {urgentNotes.length > 0 && (
        <div className="space-y-2">
          {urgentNotes.map((n) => (
            <div
              key={n.id}
              className="flex items-start gap-2.5 rounded-xl border border-red-500/30 bg-red-500/5 px-4 py-3 text-sm text-red-700"
            >
...
function AltVersions({ card }: { card: InterviewCardWithMaterials }) {
  ...
  return (
    <div className="flex flex-wrap gap-x-6 gap-y-1 border-t border-border/20 pt-3">
      {versions.map((v) => (
        <p key={v.label} className="text-sm text-muted-foreground">
  ...
  return (
    <div className="border-t border-border/20 pt-3">
      <button
        ...
        className="flex w-full items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground"
      >
        ...
        <div className="mt-3 space-y-4">
          ...
          {hasTransition && (
            <div className="rounded-lg border border-border/20 bg-muted/5 px-3 py-2">
              <span className="text-[10px] font-semibold text-muted-foreground ml-2">خروج</span>
...
function GRow({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  return (
    <div className={cn("text-sm leading-relaxed", warn ? "text-amber-700/80" : "text-muted-foreground/80")}>
      <span className="text-[10px] font-semibold ml-2 text-muted-foreground">{label}</span>
      {value}
    </div>
  )
}
...
function EntryPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-muted/10 px-3 py-2 text-sm text-muted-foreground">
          ...
          {sorted.map((m) => (
            <div key={m.id} className="rounded-lg bg-muted/10 px-3 py-2.5 text-xs">
              <div className="flex items-center gap-2">
                ...
                {m.is_pinned && <Pin className="h-2.5 w-2.5 text-amber-700" />}
                {m.credibility === "verified" && (
                  <span className="text-[9px] text-emerald-700">✓ موثق</span>
                )}
              </div>
              <p className="mt-1.5 text-muted-foreground leading-relaxed">{m.content}</p>
              {m.source_name && (
