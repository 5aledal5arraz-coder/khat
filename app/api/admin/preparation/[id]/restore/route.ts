...
import { getPreparationById, restorePreparation } from "@/lib/preparation/queries"
...
export async function POST(
  ...
  if (!prep.archived_at) return errorResponse("غير مؤرشف", 409)

  const updated = await restorePreparation(id)
  if (!updated) return errorResponse("فشل الاسترجاع", 500)
