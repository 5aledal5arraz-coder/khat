...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // NOT `/partners/${filename}`. That path is served by Next out of
    ...
    // rebuild in between. `/uploads/partners/...` is a route handler that
    // reads the file when it is asked for, so a logo works the moment it
    // lands. See app/uploads/partners/[file]/route.ts.
