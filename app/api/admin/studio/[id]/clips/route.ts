...
export async function POST(
  ...
  try {
    ...
    const saved = await createClips(id, {
      status: "ready",
...
/**
 * PATCH /api/admin/studio/[id]/clips — save admin edits to clips
 */
export async function PATCH(
  ...
  if (!existing) {
    return NextResponse.json({ error: "لا توجد مقاطع لهذه الجلسة" }, { status: 404 })
  }
  ...
  try {
    ...
    const result = await updateClips(existing.id, { clips: body.clips })
    if (!result.success) {
