...
export function ArticleDetail({ article, relatedArticles }: ArticleDetailProps) {
  ...
  const updateProgress = useCallback(() => {
    if (!articleRef.current || !article.id) return
    ...
    setArticleProgress(article.id, progress)
  }, [article.id])
  ...
  }, [mounted, article.id, updateProgress])
  ...
  const handleAddComment = () => {
    ...
    const displayName = profile?.display_name || user?.email?.split("@")[0] || "مجهول"
  ...
  return (
    <>
      ...
      <div className="fixed top-0 start-0 end-0 z-50 h-1 bg-muted" role="progressbar" aria-valuenow={readingProgress} aria-valuemin={0} aria-valuemax={100} aria-label="تقدم القراءة">
        <div
      ...
      <div className="flex flex-col gap-8 lg:flex-row">
        ...
        <article ref={articleRef} className="flex-1 min-w-0">
          ...
          <div className="mt-4 flex items-center justify-between border-t border-b py-4">
            ...
            <Button variant="outline" onClick={handleShare} className="gap-2" aria-label="مشاركة المقال">
          ...
          <Card className="mt-8">
            <CardContent className="flex items-center gap-4 p-6">
              ...
              <Button
                ...
                className="gap-2"
                aria-label={following ? `إلغاء متابعة ${article.author.name}` : `متابعة ${article.author.name}`}
              >
                {following ? (
                  <>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <button
                            className="mt-2 flex items-center gap-1 text-xs text-muted-foreground hover:text-red-500 transition-colors"
                            aria-label={`إعجاب بتعليق ${comment.authorName}`}
                          >
        ...
        <aside className="w-full shrink-0 space-y-6 lg:w-80">
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="p-4 text-center">
              ...
              <Button onClick={handleShare} className="mt-3 w-full gap-2" aria-label="مشاركة المقال">
                <Share2 className="h-4 w-4" />
                مشاركة المقال
              </Button>
            </CardContent>
