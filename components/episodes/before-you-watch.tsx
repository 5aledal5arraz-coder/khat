...
export function BeforeYouWatch({ data }: BeforeYouWatchProps) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">قبل ما تشوف</h2>
      <div className="grid gap-4 sm:grid-cols-3">
        {who_is_it_for && (
          <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-4">
            <h3 className="mb-2 text-sm font-medium text-green-600 dark:text-green-400">لمن هالحلقة؟</h3>
            <p className="text-sm leading-relaxed text-muted-foreground whitespace-pre-line">{who_is_it_for}</p>
          </div>
        )}
        {who_is_it_not_for && (
          <div className="rounded-lg border border-orange-500/20 bg-orange-500/5 p-4">
            <h3 className="mb-2 text-sm font-medium text-orange-600 dark:text-orange-400">مو لك إذا...</h3>
            <p className="text-sm leading-relaxed text-muted-foreground whitespace-pre-line">{who_is_it_not_for}</p>
          </div>
        )}
        {what_you_gain && (
          <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-4">
            <h3 className="mb-2 text-sm font-medium text-blue-600 dark:text-blue-400">شنو بتطلع فيه؟</h3>
